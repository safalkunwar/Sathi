...
async function seed() {
  ...
  if (existing.size === 0) {
    for (const companion of companions) {
      await setDoc(doc(db, 'companions', companion.id), companion);
    }
  } else {
    console.log(`Seed skipped: ${existing.size} companion documents already exist.`);
  }

  const storiesRef = collection(db, 'stories');
  const existingStories = await getDocs(storiesRef);
